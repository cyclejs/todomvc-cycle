...
function todoItemComponent(drivers) {
  var intent = {
    ...
    stopEdit$: drivers.DOM.get('.edit', 'keyup').filter(function (ev) {
      return ev.keyCode === _utils.ESC_KEY || ev.keyCode === _utils.ENTER_KEY;
    }).merge(drivers.DOM.get('.edit', 'blur')).map(function (ev) {
  ...
  return {
    DOM: vtree$,
    events: {
      destroy: intent.destroy$.withLatestFrom(props$, function (ev, _ref2) {
        var todoid = _ref2.todoid;
        return todoid;
      }),
      toggle: intent.toggle$.withLatestFrom(props$, function (ev, _ref3) {
        var todoid = _ref3.todoid;
        return todoid;
      }),
      newContent: intent.stopEdit$.withLatestFrom(props$, function (content, _ref4) {
        var todoid = _ref4.todoid;
        return { content: content, id: todoid };
      })
    }
  };
}
